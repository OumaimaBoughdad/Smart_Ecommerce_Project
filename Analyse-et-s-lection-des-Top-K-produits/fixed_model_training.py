...
def preprocess_data(input_file):
    ...
    for col in numeric_cols:
        df[col].fillna(df[col].mean() if not df[col].isna().all() else 0, inplace=True)
    ...
    if 'note_moyenne' in df.columns:
        ...
        df['note_moyenne'].fillna(0, inplace=True)
    else:
        df['note_moyenne'] = 0
    ...
    if 'prix' not in df.columns or df['prix'].isna().all():
        df['prix'] = 100  # Default price
    ...
    if len(available_features) > 0 and len(df) > 0:
        scaler = MinMaxScaler()
        df[available_features] = scaler.fit_transform(df[available_features])
    
    # Invert price (lower is better)
    df['prix_inv'] = 1 - df['prix'] if 'prix' in df.columns else 0.5
